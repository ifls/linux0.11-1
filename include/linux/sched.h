...
// unsigned short suid 保存的用户id。 saved user_id
// unsigned short gid 组标识号（组id）。
// unsigned short egid 有效组id。
// unsigned short sgid 保存的组id。 saved group_id
...
struct task_struct
{
	...
	long pid;
	long father;
	long pgrp; // 进程组id, 好像只影响 终端相关的 信号掩码和信号处理
	long session;
	long leader;
	// leader = 1 说明是 会话的领导者
	...
	unsigned short umask;	  // 创建新文件的权限掩码
	//
	struct m_inode *pwd; // 进程工作目录 process work dir
	struct m_inode *root;
	struct m_inode *executable;
	unsigned long close_on_exec;
	struct file *filp[NR_OPEN];
	/* ldt for this task 0 - zero 1 - cs 2 - ds&ss */
	struct desc_struct ldt[3]; // 本地描述表
							   /* tss for this task */
	struct tss_struct tss;	   // tss 段
};
...
extern long startup_time;						// 开机时间。从1970:0:0:0 开始计时的秒数。 也就是unix时间戳
...
extern _inline void switch_to(int n)
{
	...
	__tmp = (unsigned short)_TSS(n); // n号进程的tss描述符的地址
	...
	_asm {
		...
		// 用emit就是在当前位置直接插入数据（实际上是指令），一般是用来直接插入汇编里面没有的特殊指令，多数指令可以用asm内嵌汇编来做，没有
		_emit 0xea
		_emit 0 // ip
		_emit 0 
		_emit 0 
...
__asm__( "cmpl %%ecx,_current\n\t" \
  ...
  "movw %%dx,%1\n\t" \
  "xchgl %%ecx,_current\n\t" \	。
  "ljmp %0\n\t" \		。 // 如果 ljmp 指令后面跟的是一个 tss 段，那么，会由硬件将当前各个寄存器的值保存在当前进程的 tss 中，并将新进程的 tss 信息加载到各个寄存器。
// 在任务切换回来后才会继续执行下面的语句。
...
extern _inline void _set_base(unsigned short *addr, unsigned long base)
{
	/*	addr[1] = base;
		((char*)addr)[4] = base >> 16;
		((char*)addr)[7] = base >> 8;*/
	_asm mov ebx, addr _asm mov edx, base _asm mov word ptr[ebx + 2], dx // 基址base 低16 位(位15-0)->[addr+2]。
		_asm ror edx,
		16 // edx 中基址高16 位(位31-16) -> dx。
		_asm mov byte ptr[ebx + 4],
		dl // 基址高16 位中的低8 位(位23-16)->[addr+4]。
		_asm mov byte ptr[ebx + 7],
		dh // 基址高16 位中的高8 位(位31-24)->[addr+7]。
}
/*
__asm__( "movw %%dx,%0\n\t" \
"rorl $16,%%edx\n\t" \
...
extern _inline void _set_limit(unsigned short *addr, unsigned long limit)
{
	...
	_asm mov ebx, addr _asm mov edx, limit _asm mov word ptr[ebx], dx // 段长limit 低16 位(位15-0)->[addr]。
		_asm ror edx,
		16 // edx 中的段长高4 位(位19-16)->dl。
		_asm mov dh,
		byte ptr[ebx + 6] // 取原[addr+6]字节->dh，其中高4 位是些标志。
		_asm and dh,
		0f0h // 清dh 的低4 位(将存放段长的位19-16)。
			_asm or
			dl,
		dh // 将原高4 位标志和段长的高4 位(位19-16)合成1 字节，
		_asm mov byte ptr[ebx + 6],
		dl // 并放回[addr+6]处。
}
/*
