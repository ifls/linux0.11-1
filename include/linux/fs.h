...
{
  _asm mov ebx, head _asm inc dword ptr[ebx] _asm and dword ptr[ebx], 4095
}
...
struct buffer_head
{
  char *b_data; /* pointer to data block (1024 bytes) */ // 指向数据块的指针。
...
struct m_inode
{
  ...
  unsigned short i_uid;       // 用户id（文件拥有者标识符）。
  unsigned long i_size;       // 文件大小（字节数）。
  unsigned long i_mtime;      // modify time 修改时间（自1970.1.1:0 算起，秒）。
  unsigned char i_gid;        // 组id(文件拥有者所在的组)。
  unsigned char i_nlinks;     // 文件目录项链接数。
  unsigned short i_zone[9];   // 数据块 直接(0-6)、间接(7)或双重间接(8)逻辑块号。
