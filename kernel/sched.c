...
// jiffies 是系统从开机开始算起的滴答数（10ms/滴答）。定义在sched.h 第139 行。
			...
			{
				(*p)->signal |= (1 << (SIGALRM - 1)); // 记上信号, 下面把进程变为就绪态
				(*p)->alarm = 0;
			}
...
// 指向哪个的任务号。
      ...
      // counter 值的计算方式为counter = counter /2 + priority。[右边counter=0??]
		for (p = &LAST_TASK; p > &FIRST_TASK; --p)
			if (*p)
				(*p)->counter = ((*p)->counter >> 1) + (*p)->priority; // 更新剩余执行时间
	}
...
// 下面代码用于初始化8253 定时器。
  ...
  // 设置时钟中断处理程序句柄（设置时钟中断门）。 关乎调度
	set_intr_gate (0x20, &timer_interrupt); 
  // 修改中断控制器屏蔽码，允许时钟中断。0x 1111 1110
