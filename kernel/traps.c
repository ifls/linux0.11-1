...
// 断点陷阱中断int3、溢出中断overflow 和边界出错中断bounds 可以由任何程序产生。可以由用户态触发?
// 这两个函数均是嵌入式汇编宏程序(include/asm/system.h,第36 行、39 行)。
void trap_init(void)
{
	int i;

	set_trap_gate(0,&divide_error);// die 设置除操作出错的中断向量值。以下雷同。
	set_trap_gate(1,&debug); // die
	set_trap_gate(2,&nmi); // die
	set_system_gate(3,&int3);	// 只是打印 一些信息 /* int3-5 can be called from all */
	set_system_gate(4,&overflow); // die
	set_system_gate(5,&bounds); // die
	set_trap_gate(6,&invalid_op); // die
	set_trap_gate(7,&device_not_available); // die
	set_trap_gate(8,&double_fault); // die
	set_trap_gate(9,&coprocessor_segment_overrun); // die
	set_trap_gate(10,&invalid_TSS); // die
	set_trap_gate(11,&segment_not_present); // die
	set_trap_gate(12,&stack_segment); // die
	set_trap_gate(13,&general_protection); // die
	set_trap_gate(14,&page_fault); // 缺页中断
	set_trap_gate(15,&reserved); // die
	set_trap_gate(16,&coprocessor_error); // die 
// 下面将int17-48 的陷阱门先均设置为reserved，以后每个硬件初始化时会重新设置自己的陷阱门。
	for (i=17;i<48;i++)
		set_trap_gate(i,&reserved); // die
	set_trap_gate(45,&irq13);// 设置协处理器的陷阱门。协处理器中断处理 coprocessor_error
	outb_p(inb_p(0x21)&0xfb,0x21);// 允许主8259A 芯片的IRQ2 中断请求。
	outb(inb_p(0xA1)&0xdf,0xA1);// 允许从8259A 芯片的IRQ13 中断请求。
	set_trap_gate(39,&parallel_interrupt);// 设置并行口的陷阱门。中断函数里只是发送end of interupt 信号
