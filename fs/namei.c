...
// 如果没有定义NO_TRUNCATE，则若文件名长度超过最大长度NAME_LEN，则截短之。
	...
	entries = (*dir)->i_size / (sizeof(struct dir_entry)); // 成倍
	*res_dir = NULL;
	...
	if (!(bh = bread((*dir)->i_dev, block))) // buffer_head*
		return NULL;
	...
	while (i < entries)
	{
		// 如果当前目录项数据块已经搜索完，还没有找到匹配的目录项，则释放当前目录项数据块。下面再读取下一个块
